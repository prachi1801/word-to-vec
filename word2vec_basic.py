...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  while(data_index < data_len and total < batch_size) :
    context_words = []
    st = max(data_index - skip_window, 0)
    end = min(data_len, data_index + 1 + skip_window)
    for j in range(st, end) :
      if j != data_index:
        context_words.append(j)
    con_len = len(context_words)
    final_context_words = random.sample(context_words,min(con_len, num_skips))
    for word in final_context_words :
      if total < batch_size :
        batch[total] = data[data_index]
        labels[total] = data[word]
        total += 1
      else:
        break
    data_index += 1
